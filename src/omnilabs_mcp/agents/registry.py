"""Agent registry — auto-discovers YAML agent definitions.

Discovery order (later overrides earlier):
  1. agents/builtin/*.yaml  (ship with OmniLabs, contributors add PRs here)
  2. ~/.omnilabs/agents/*.yaml  (user-local overrides and custom agents)
"""
...
def _parse_yaml(text: str) -> dict:
    ...
    while i < len(lines):
        ...
        if value == "|":
            ...
            # Plain string value — strip surrounding quotes
...
def _load_yaml_agent(path: Path) -> AgentSpec:
    """Parse a YAML file into an AgentSpec."""
...
def _discover_from_dir(registry: AgentRegistry, directory: Path, label: str) -> None:
    """Discover all .yaml files in a directory and register them."""
...
def build_registry() -> AgentRegistry:
    ...
    _discover_from_dir(registry, BUILTIN_DIR, "builtin")
